...
par_sin1 = [314, 1.57, 312, 360]#per fit
...
if(bool_fit):##mancano residui
    ...
    erroref = (domega0)/(2*pylab.pi)
    print("Quindi f aspettata è %.20f +- %f " %((omega0/(2*pylab.pi)), erroref))
    pylab.show()
if(bool_filter_low):
    coeff = -1000.
    ...
    pylab.title("Trasformata inversa(con filtro passa basso)")
    pylab.xlabel("Tempo [s]")
    pylab.ylabel("d.d.p. [u.a.]")
    pylab.xlim(TLIM)
    print(len(ampiezza_t))
    pylab.grid(color = "gray")
    pylab.grid(b=True, which='major', color='#666666', linestyle='-')
    pylab.grid(b=True, which='minor', color='#999999', linestyle='-', alpha=0.2)
    pylab.minorticks_on()
    ...
    pylab.show()
    pylab.xlim(FLIM)
    pylab.title("FFT(con filtro passa basso)")
    pylab.xlabel("Frequenza [Hz]")
    pylab.ylabel("Ampiezza in dB")
    pylab.grid(color = "gray")
    pylab.grid(b=True, which='major', color='#666666', linestyle='-')
