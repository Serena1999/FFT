...
# --> choosen file::
FileName = "sin11.txt"
#FileName = "sin12.txt" #di questa parlane: manca periodicità
#FileName = "sin14.txt"
#FileName = "sin15.txt" #di questa di che l'elevata frequenza porta ad
#un sottocampionamento nel periodo
#FileName = "sin20.txt"
#FileName = "sin30.txt"
#FileName = "sin31.txt"
#FileName = "sin32.txt"
#FileName = "sin33.txt"
Directory = "data_sinusoidale_F/"

# --> fit  initial parameters::
#par_sin1 = [209, 1.57, 407, 390]#per fit sin11.txt
#per sin12 no fit
par_sin1 = [22500, 1.57, 418, 400]#per fit sin14.txt sistema sei hai tempo
#per sin15 no fit
...
if(bool_fit):##mancano residui
    ...
    g1.set_xlim(TLIM)
    g2.set_xlim(TLIM)
    g2.set_xlabel("Tempo [s]")
    g1.set_ylabel("d.d.p. [digit]")
    g1.set_ylabel("Residui normalizzati")
    ...
    print("q = %f +- %f" %(q0, dq0))
    print("A = %f +- %f" %(A0, dA0))
    Corr_omega_phi = pcov[1][0]/pylab.sqrt(pcov[1][1] * pcov[0][0])
    Corr_omega_q = pcov[2][0]/pylab.sqrt(pcov[2][2] * pcov[0][0])
    Corr_omega_A = pcov[3][0]/pylab.sqrt(pcov[3][3] * pcov[0][0])
    Corr_phi_q = pcov[2][1]/pylab.sqrt(pcov[2][2] * pcov[1][1])
    Corr_phi_A = pcov[3][1]/pylab.sqrt(pcov[3][3] * pcov[1][1])
    Corr_q_A = pcov[2][3]/pylab.sqrt(pcov[3][3]* pcov[2][2])
    print("Corr_omega_phi = %f" %Corr_omega_phi)
    print("Corr_omega_q = %f" %Corr_omega_q)
    print("Corr_omega_A = %f" %Corr_omega_A)
    print("Corr_phi_q = %f" %Corr_phi_q)
    print("Corr_phi_A = %f" %Corr_phi_A)
    print("Corr_q_A = %f" %Corr_q_A)
    g1.errorbar(t, y, dy, dt, linestyle = '', marker = '.', color = 'black')
    x_plot = numpy.linspace(*TLIM, 1000)
    g1.plot(x_plot, f_sin(x_plot, *popt), color = "red")
    g1.minorticks_on()
    g1.grid(color = "gray")
    g1.grid(b=True, which='major', color='#666666', linestyle='-')
    g1.grid(b=True, which='minor', color='#999999', linestyle='-', alpha=0.2)
    ...
    g2.grid(b=True, which='major', color='#666666', linestyle='-')
    g2.grid(b=True, which='minor', color='#999999', linestyle='-', alpha=0.2)
    residui = numpy.zeros(len(t))
    for i in range(len(t)):
        residui[i] = y[i] - f_sin(t[i], *popt)
    chi = (residui**2/dw).sum()
    print("chi ottenuto = %f" %chi)
    chi_aspettato = len(t) - len(popt)
    print("chi aspettato = %f" %chi_aspettato)
    chi_ridotto = chi/chi_aspettato
    print("chi ridotto = %f"%chi_ridotto)
    residui_normalizzati = numpy.zeros(len(residui))
    residui_normalizzati = residui/residui.sum()
    g2.errorbar(t, residui_normalizzati, dw/chi, linestyle = '--', marker = '.', color = 'black')
