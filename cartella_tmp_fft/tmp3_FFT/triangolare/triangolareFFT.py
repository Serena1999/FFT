...
if(bool_single_FFT):
    if(bool_lin):
        ...
        pylab.title("FFT onda triangolare")
        if(bool_focus_FFT):
            pylab.xlim(FFOCUS)
        else:
            pylab.xlim(FLIM)
...
if(bool_filter_low):
    ...
    pylab.title("Simulazione di integratore con onda triangolare in ingresso")
...
if(bool_filter_high):
    ...
    pylab.grid(b=True, which='minor', color='#999999', linestyle='-', alpha=0.2)
    pylab.xlim(min(t), max(t))
    pylab.title("Simulazione di derivatore con onda triangolare in ingresso")
    pylab.xlabel("Tempi [s]")
    pylab.ylabel("d.d.p. [digit]")
    pylab.minorticks_on()
    ampiezza_t = abs(numpy.fft.irfft(gfilter))
